...
(cd extensions/cursor && bun install && bun test && bunx tsc --noEmit)
scripts/qa/check-loc.sh

if find crates extensions/cursor/src -type f | xargs grep -n 'xterm\|xterm.js' >/tmp/stp-no-browser-check.txt 2>/dev/null; then
...
./target/debug/stp open-cursor --registry "$TMP_DIR/registry.json" --terminal-id 00000000-0000-0000-0000-000000000401 --dry-run --log "$EVIDENCE/C003-open-cursor.log"
grep 'cursor --new-window' "$EVIDENCE/C003-open-cursor.log"
